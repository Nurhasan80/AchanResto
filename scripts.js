...
function validatePhoneNumber(phone) {
  ...
  // Cek awalan nomor (harus 0 atau 62)
  if (cleaned.startsWith('0')) {
    // Jika awalan 0, panjang harus 10-12
    return cleaned.length >= 10 && cleaned.length <= 12;
  } else if (cleaned.startsWith('62')) {
    // Jika awalan 62, panjang harus 11-13
...
function formatRupiah(angka) {
  ...
  // Format dengan Intl.NumberFormat untuk hasil yang lebih konsisten
...
function validateForm() {
  ...
  // Aktifkan tombol jika semua field wajib terisi dan telepon valid
  if (name && isPhoneValid && address && quantity) {
...
function sendToWhatsApp(e) {
  ...
  // Validasi telepon sekali lagi sebelum mengirim
  const phone = document.getElementById('phone').value.trim();
  if (!validatePhoneNumber(phone)) {
      alert('Format nomor telepon tidak valid. Harus diawali 0 atau 62, dan panjang 10-13 digit.');
      return;
  }
  ...
  message += `Harga: Rp ${formatRupiah(productPrice)}%0A`;
  message += `Jumlah: ${quantity} box%0A`;
  message += `Total: Rp ${formatRupiah(total)}%0A%0A`;
...
// Event listener untuk validasi telepon saat input
...
// Event listener untuk validasi telepon saat kehilangan fokus
